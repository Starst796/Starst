    ...
    try:
        ...
        for membership in other_memberships:
            if membership.room.creator_id == current_user.id:
                membership.room.is_active = False
                RoomMember.query.filter_by(room_id=membership.room.id).delete()
            else:
...
def leave_room_http(current_user, room_id):
    ...
    if not membership:
        return jsonify({'message': '您不是这个房间的成员！'}), 404
...
def get_room_chat(current_user, room_id):
    ...
    try:
        # 获取最新的50条消息，按时间降序排列
        messages = RoomChat.query.filter_by(room_id=room_id)\
            .order_by(RoomChat.created_at.desc())\
            .limit(50)\
            .all()
        
        # 将消息列表反转，使最旧的消息在前，最新的消息在后
        messages.reverse()
        
        # 计算最新消息的ID
        latest_message_id = 0
        if messages:
            latest_message_id = messages[-1].id  # 最后一条消息是最新的
...
def handle_withdraw_chat(current_user, data):
    ...
    try:
        ...
        db.session.commit()
        # 广播消息撤回事件
        emit('withdraw_message', {'message_id': message_id}, to=str(chat_message.room_id))
...
def check_and_create_match(match_text, player_required):
    ...
    try:
        ...
        waiting_users = MatchUser.query.filter_by(
            ...
            player_required=player_required,
            is_matched=False
        ).order_by(MatchUser.created_at.asc()).all()
        
        if len(waiting_users) >= player_required:
            # 选择前N个用户进行匹配
            matched_users = waiting_users[:player_required]
            
            # 创建房间
            room_name = f"匹配房间-{match_text}"
            creator = matched_users[0].user
            ...
            # 添加所有匹配用户到房间
            for i, match_user in enumerate(matched_users):
                membership = RoomMember(
                    user_id=match_user.user_id,
                    room_id=new_room.id,
                    is_ready=True if i == 0 else False  # 第一个用户（房主）设为准备状态
                )
                db.session.add(membership)
                
                # 标记为已匹配并设置房间ID
                match_user.is_matched = True
                match_user.room_id = new_room.id
            ...
            socketio.emit('match_success', {
                'room': room_data,
                'match_text': match_text,
                'matched_users': [match_user.user_id for match_user in matched_users]
            }, to='0')
            
            # 通知房间列表更新（新房间创建）
            notify_room_list_update()
            
            print(f"匹配成功：{match_text}，创建房间 {new_room.id}，玩家数：{len(matched_users)}")
...
def notify_room_list_update():
    ...
    try:
        ...
        socketio.emit('room_list_update', {
            'rooms': rooms_data,
            'timestamp': datetime.now(east8).isoformat()
        }, to='0')
        
        print(f"发送房间列表更新通知，房间数量: {len(rooms_data)}")
    except Exception as e:
        print(f"发送房间列表更新通知失败: {str(e)}")

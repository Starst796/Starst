...
def create_room(current_user):
    ...
    try:
        ...
        for membership in other_memberships:
            leave_room_db(current_user, membership.room.id)
...
def tips_update_online_users(user_id):
    ...
    #如果user在房间中，向该房间发送提示：在线人数可以更新了
    memberships = RoomMember.query.filter_by(user_id=user_id).all()
    for membership in memberships:
        if membership.room.is_active:
            socketio.emit('update-online-members', {'room_id': membership.room_id,'user_id': user_id}, to=str(membership.room_id))
